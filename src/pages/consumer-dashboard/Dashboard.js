...
const Dashboard = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery(["financial-history"], fetchHistory);
  console.log("query data is: ", historyData?.data);
  console.log(
    "query specific data is: ",
    historyData?.data?.data?.loan_applications?.data
  );
          ...
          <div className="hidden lg:block  lg:w-2/6 ">
            <div className="w-full">
              <LargeCard classNames="w-full">
                ...
                {/* history's component  */}
                {historyData?.data?.data?.loan_applications?.data?.length ? (
                  <>
