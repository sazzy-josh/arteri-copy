...
const Loans = () => {
  ...
  const fetchHistory = (page) => {
    return axios.get(
      `${process.env.REACT_APP_BASE_URI}/loan/history/get?page=${page}`,
  ...
  const {
    ...
    isSuccess,
    isFetching,
  } = useQuery(["fetch-historyData", page], () => fetchHistory(page), {
    onSuccess: (data) => {
      console.log(data);
    },
    staleTime: Infinity,
    ...
    <>
      ...
      <section className=' mb-8 px-3 py-5 sm:shadow-2xl sm:shadow-[#EAF2FB] md:overflow-auto md:w-[95%] md:mx-auto md:px-0 md:mb-12'>
          ...
          <tbody>
            {isLoading || isFetching
              ...
              : isSuccess &&
                ...
                historyData?.data?.data?.loan_applications?.data.map(
                  (item, index) => (
                    <tr
                      onClick={() =>
                    ...
                    >
                      ...
                      <td className='p-[18px] whitespace-nowrap font-medium hidden lg:table-cell'>
                        {item?.extended_details?.loan_information?.amount_requested.toLocaleString() ??
                          "- - -"}
                      </td>
                      <td className='p-[18px] whitespace-nowrap font-medium  '>
                        <p
        ...
        {!isFetching &&
          !isLoading &&
          isSuccess &&
          historyData?.data?.data?.loan_applications?.data.length === 0 && (
      ...
      <div className='flex flex-col lg:flex-row mb-5'>
        <div className='flex gap-x-2 px-5 items-center mb-14 lg:mb-0'>
          <span className='border-r border-r-black pr-2'>
            {historyData?.data?.data?.loan_applications?.total}{" "}
            {historyData?.data?.data?.loan_applications?.total === 1
              ? "Result"
              : "Results"}
          </span>
          ...
          <div className='flex flex-row'>
            ...
            >
              {historyData?.data?.data?.loan_applications?.links
                .slice(
                  1,
                  historyData?.data?.data?.loan_applications?.links?.length - 1,
                )
                .map((item) => {
                  return (
        ...
        <div className='flex flex-col gap-x-10 lg:flex-row lg:ml-auto px-5 gap-y-5 -mt-[12px]'>
          <div className='w-full lg:w-28 h-14'>
            <SecondaryButton
              ...
              isButtonDisabled={
                historyData?.data?.data?.loan_applications?.prev_page_url ===
                null
              }
            >
              Back
          ...
          <div className='w-full  lg:w-28 h-14'>
            <PrimaryButton
              ...
              isButtonDisabled={
                historyData?.data?.data?.loan_applications?.next_page_url ===
                null
              }
            >
              Next
